...
export default async (client: Client, game: runningUnoGame): Promise<MessageCreateOptions> => {
    ...
    return {
        content: game.guildApp ? game.currentPlayer.startsWith("AI-") ? t("strings:game.ai", { name: await getUsername(client, game.guildId, game.currentPlayer, !game.guildApp) }) : t("strings:game.mention", { mention: `<@${game.currentPlayer}>`, lng }) : game.previousActions?.reverse().slice(0, maxActionShownInUserApp).reverse().join("\n"),
